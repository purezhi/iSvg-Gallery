...
iSvgGallery.app = function (photos) {
    ...
    var SVG = "http://www.w3.org/2000/svg", // SVG命名空间
        ...
        globalCache.tooltip = {}, // 程序缓存区变量，放tooltip定时函数等
        ...
        imgZoomMinWidth = 320, // 图片缩放最小宽度
        imgZoomMinHeight = 280, // 图片缩放最小高度
        imgInitWRange = [imgZoomMinWidth, 350], // 图片初始化到画廊上时的宽度区间(galleryViewWidth/galleryMinWidth > 1时，最小值和最大值乘以该倍数)
        imgInitHRange = [imgZoomMinHeight, 300], // 图片初始化到画廊上时的高度区间(galleryViewHeight/galleryMinHeight > 1时，最小值和最大值乘以该倍数)
        ...
        imageBorderStrokeOpacity = 1.0, // 图片笔触透明度
        imageDefaultOpacity = 1.0,
...
//TODO IE 9中，图片获得焦点并被带到最前方时(bringToFront)时，不能响应画廊(canvas)的单击事件
        // 鼠标双击时: 显示原图
        g.addEventListener(
                "dblclick", 
                function(evt) { 
        ...
        var rsz = (source.width/2-60)<hotSpotSide ? (source.width/2-50) : hotSpotSide; // 操作热区(rect)的长宽值
        rsz = (source.height/2-60)<rsz ? (source.height/2-50) : rsz;
        rsz= rsz<20 ? 20 : rsz; // 最小十像素
    ...
    function onEnterCover(ev) {
        ...
        if(album && album.name) {
            renderTooltip(ev, album.name, false, 30000);
        }
    ...
    function onMouseMove(ev) {
        ...
        if(baseName(ev).indexOf(coverIdPrefix) == -1 && 
            ...
            globalCache.tooltip = {}; // 清空缓存区
        }

        // 如果封面被移动，移除其上的提示框
        if(baseName(ev).indexOf(coverIdPrefix) == 0 && 
                globalCache && 
                globalCache.tooltip) {
            // 删除定时函数和清空在显示的提示框
            for(var f in globalCache.tooltip) {
                var tt = document.getElementById(globalCache.tooltip[f].id); // 提示框元素
                if(currentTransform && globalCache.tooltip[f].src == parentAlbumName(baseName(ev)) && tt) { // 如果封面被移动，移除其上的提示框
        ...
        item.addEventListener(
                ...
                function(id) {
                        return function(){  
                                ...
                                if(bdr) bdr.setAttribute("class", "ctxMenuItemHover");
                                if(txt) txt.setAttribute("class", "ctxMenuItemTextHover");
                        };
                }(itemId),
                false
        );
        ...
        item.addEventListener(
                ...
                function(id) {
                        return function(){  
                                ...
                                var bdr = document.getElementById(gId+"-border"); // 菜单项目容器组(g)边框
                                var txt = document.getElementById(gId+"-text"); // 菜单项目容器组(g)文本
                                if(bdr) bdr.setAttribute("class", "ctxMenuItem");
                                if(txt) txt.setAttribute("class", "ctxMenuItemText");
                        };
                }(itemId),
                false
        );
    ...
    function createDividingLine(x1, y1 , x2, y2) {
        ...
        ln.setAttribute("class", "ctxDividingLine");
...
//TODO 关闭溢出通知
    ...
    function renderBackgroundOverlay(create, evt, handler) {
        var pId = "top-img-background-overlay"; // 背景遮盖层元素ID
    ...
    function renderTooltip(evt, content, rightNow, time) {
        ...
        var pId = aId + "-tooltip"; // 背景遮盖层元素ID
        var pFun = aId + "-tooltip-function"; // 背景遮盖层元素ID
        ...
        if(!rightNow) {
            ...
            globalCache.tooltip[pFun].src = aId;
            globalCache.tooltip[pFun].id = pId;
        ...
        if(cover && album && cover.vTranslate && album.vTranslate) {
            var px = cover.vTranslate[0] + album.vTranslate[0];
            var py = cover.vTranslate[1] + album.vTranslate[1];
        } else {
            var px = evt.clientX;
            var py = evt.clientY;
        }
        ...
        rectBg.setAttribute("class", "tooltipBackground");
        ...
        // 显示一定时间后删除元素
        setTimeout(function(){ 
                document.getElementById(galleryElId).removeChild(tooltip); 
            }, 
            (time?time:3000)
        ); // 定时递归调用

        // 从程序缓存区删除定时函数
        delete globalCache[pFun];
