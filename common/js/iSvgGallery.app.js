...
iSvgGallery.app = function (photos) {
    ...
    var SVG = "http://www.w3.org/2000/svg", // SVG命名空间
        ...
        galleryElBgId = "background-rect"; // 画廊背景元素ID
        ...
        imageTypeCover = "cover", // 图片类型：相册封面
        imageTypeImage = "image", // 图片类型：相册图片
        imageTypeNormal = "normal", // 图片类型：普通，与相册无关
        ...
        imagesOnGallery = [], // 当前在画廊上展示的图片对应的顶层容器组(g)的ID数组 {id: imageIdPrefix+ID, uri: image_url, type: imageTypeCover/..., album: albumIdPrefix+ID,  position: grid_position}
        ...
        globalArea = {}, // 程序缓存区，放临时变量
        globalArea.noNeedLoad = false, // 当清除画廊显示时，没有加载的图片不再加载
        globalArea.tooltip = {}, // 程序缓存区变量，放tooltip定时函数等(变量名：提示框元素ID + "-function" = {src:提示框对应的元素ID, id:提示框元素ID, func:提示框定时函数})
        globalArea.opacity = {}, // 程序缓存区变量，放元素透明度定时函数等(变量名：透明度变化元素ID + "-function"={id:透明度变化元素ID, func:透明度变化定时函数})
        globalArea.debug = "", // 程序缓存区，放调试信息
...
document.getElementById(galleryElId).addEventListener(
    "dblclick", 
    function(ev){ 
        if(ev.target.id.indexOf(coverIdPrefix)==0 || ev.target.id.indexOf(imageIdPrefix)==0 || ev.target.id.indexOf("top-img")==0) {
            return true;
        }  
        alert(globalArea.debug); globalArea.debug = ""; 
    }, 
    false
);*/
        document.getElementById(galleryElBgId).addEventListener("mousemove", onMouseMove, false);
        document.getElementById(galleryElBgId).addEventListener("mouseup", onMouseUp, false);
    ...
    function displayAlbum(k, album, param) {
        ...
        // 把图片类型(cover)，相册ID设定到相册配置
        param.type = imageTypeCover;
        param.album = almIdStr;
        // 初始图片唯一ID
    ...
    // param: 为空时，为普通图片。
    //            不为空，param.type=image时，为相册图片; param.type=cover时，为相册封面
    ///
    ...
    function displayImage(image, param) {
        ...
        img.onload = function(url, imgId) { return function() {
                // 不再加载图片
                if(globalArea.noNeedLoad !== false) return;
                // 待显示图片不是当前显示相册的图片
                if(topAlbumsOnGallery && topAlbumsOnGallery.id && 
                        (
                            !param || 
                            param && 
                            param.type && 
                            param.type == imageTypeImage &&
                            param.album &&
                            topAlbumsOnGallery.id != param.album
                        )) {
                    return;
                }
                ...
                // param被设定，且图片类型为相册封面，是封面图片
                if(param && param.type && param.type == imageTypeCover) {
                ...
                if(param && param.type && param.type == imageTypeCover) {
                    ...
                    // 把图片添加到画廊图片数组
                    if(param && param.type && param.type == imageTypeImage && param.album) {
                        imagesOnGallery.push({id: (imageIdPrefix + imgId), uri:this.src, type: imageTypeImage, album: param.album, position: posInfo.position});
                    } else {
                        imagesOnGallery.push({id: (imageIdPrefix + imgId), uri:this.src, type: imageTypeNormal, position: posInfo.position});
                    }
                }
    ...
    function addImage(source, imgId, url, initOpacity) {
        ...
        g.addEventListener(
                ...
                function(evt) { 
                        // 把目标对象放置到最前面
                        // (webkit核心浏览器，执行bringToFront后，不能响应双击事件)
                        if(!browser("webkit")) { 
                            bringToFront(s); 
                        }
    ...
    function getImageOnGallery(imgId) {
        if(imagesOnGallery instanceof Array) {
            for(var i=0; i<imagesOnGallery.length; i++) {
                if(imagesOnGallery[i].id == imgId) {
                    return imagesOnGallery[i];
                }
            }
        }
    }
    ...
    function onMouseMove(ev) {
        ...
        // 程序缓存区有提示框设定
        if(globalArea && globalArea.tooltip) {
            // 删除定时函数和清空在显示的提示框
            for(var f in globalArea.tooltip) {
                // 标鼠指针在封面上快速掠过，不显示提示框，并移除已有的提示框
                if(baseName(ev).indexOf(coverIdPrefix) == -1) {
                    clearTimeout(globalArea.tooltip[f].func); // 删除定时函数

                    var tt = document.getElementById(globalArea.tooltip[f].id); // 提示框元素
                    if(tt) { // 如果封面被移动
                        tt.parentNode.removeChild(tt); // 移除其上的提示框
                    }

                    delete globalArea.tooltip[f]; // 删除程序缓冲区数据
    ...
    function postAlbumOnGellery(id) {
        ...
        // 展示图片到画廊
        listImages(album.images, {type: imageTypeImage, album: s});
...
//TODO 关闭溢出通知
    ...
    function renderTooltip(evt, content, rightNow, time) {
        ...
        if(document.getElementById(pId) || (!rightNow && globalArea.tooltip[pFun])) {
            return;
        }

        // 不立即显示，设定再触发时间
        if(!rightNow) {
            globalArea.tooltip[pFun] = {};
            globalArea.tooltip[pFun].src = aId; // 触发元素
            globalArea.tooltip[pFun].id = pId; // 提示框元素ID
            globalArea.tooltip[pFun].func  = setTimeout(function(){ 
                    return function(){ 
        ...
        setTimeout(function(){ 
                ...
                delete globalArea.tooltip[pFun]; // 删除程序缓冲区数据
            }, 
            (time?time:3000)
        ); // 定时递归调用
    ...
    function rampOpacity(g, d, s) {
        ...
        if(globalArea.opacity[opctFuncId]) {
            // 停止透明度变化
            clearTimeout(globalArea.opacity[opctFuncId].func);
        } else{
            // 初始化透明度变化函数信息
            globalArea.opacity[opctFuncId] = {};
            globalArea.opacity[opctFuncId].id = g.id;
        }
...
//DEBUG
//globalArea.debug += "[globalArea.opacity[" + opctFuncId + "]: " + globalArea.opacity[opctFuncId] + "][rampOpacityDown] [g.style.opacity: " + g.style.opacity + "][Opacity: " + opct + "]\n\n";
        var rampFunc = function () {
            var o = parseFloat(g.style.opacity) - 0.05;
            g.style.opacity = o;

            if (o > parseFloat(opct)) {
                globalArea.opacity[opctFuncId].func = setTimeout(rampFunc, 10); // 定时递归调用
            } else { // 效果处理完毕，在全局变量域删除定时信息
                delete globalArea.opacity[opctFuncId];
            }
        }
    ...
    function rampOpacityUp(g, opct) {
        ...
        var opctFuncId = g.id+"-function";
//DEBUG
//globalArea.debug += "[globalArea.opacity[" + opctFuncId + "]: " + globalArea.opacity[opctFuncId] + "][rampOpacityUp] [g.style.opacity: " + g.style.opacity + "][Opacity: " + opct + "]\n\n";
        var rampFunc = function () {
            var o = parseFloat(g.style.opacity) + 0.05;
            g.style.opacity = o;
